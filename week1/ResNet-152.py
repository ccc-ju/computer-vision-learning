...
'''
 @param:
    in_planes:输入的通道数的大小
    out_planes:输出的通道数的大小
 A:为什么bias置为False?
 Q:当卷积层跟着bn层时候是不需要bias的，bn层会重新计算方查和偏差
'''
...
model_rate = torch.nn.functional.softmax(output[0], dim=0)
flag = 0
max_rate = model_rate[0]
flag = 0
for i in range(1, len(model_rate)):
    if model_rate[i] > max_rate:
        flag = i
        max_rate = model_rate[i]
plt.title('this picture is {}, probability is:{}'.format(labels[flag], max_rate))
plt.imshow(input_image)
plt.show()

# print(torch.max(model_rate))
# print(model_rate)